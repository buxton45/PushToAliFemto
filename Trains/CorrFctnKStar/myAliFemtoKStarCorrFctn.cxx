...
#ifdef __ROOT__
ClassImp(myAliFemtoKStarCorrFctn)
#endif

//____________________________
myAliFemtoKStarCorrFctn::myAliFemtoKStarCorrFctn(const char* title, const int& nbins, const float& KStarLo, const float& KStarHi):
  fNumerator(0),
  fDenominator(0),
  fRatio(0),
  fkTMonitor(0),
...
{
  // set up numerator
  //  title = "Num KStar (MeV/c)";
  char tTitNum[101] = "Num";
  strncat(tTitNum,title, 100);
  fNumerator = new TH1D(tTitNum,title,nbins,KStarLo,KStarHi);
  // set up denominator
  //title = "Den KStar (MeV/c)";
  char tTitDen[101] = "Den";
  strncat(tTitDen,title, 100);
  fDenominator = new TH1D(tTitDen,title,nbins,KStarLo,KStarHi);
  // set up ratio
  //title = "Ratio KStar (MeV/c)";
  char tTitRat[101] = "Rat";
  strncat(tTitRat,title, 100);
  fRatio = new TH1D(tTitRat,title,nbins,KStarLo,KStarHi);
  ...
  char tTitPairKStar[101] = "PairKStar";
  strncat(tTitPairKStar,title, 100);
  fPairKStar = new TNtuple(tTitPairKStar,title,  "KStarMag:KStarOut:KStarSide:KStarLong");
...
//____________________________
myAliFemtoKStarCorrFctn::myAliFemtoKStarCorrFctn(const myAliFemtoKStarCorrFctn& aCorrFctn) :
  AliFemtoCorrFctn(),
  fNumerator(0),
  fDenominator(0),
  fRatio(0),
...

{
  ...
  if (aCorrFctn.fPairKStar)
    fPairKStar = (TNtuple*)aCorrFctn.fPairKStar;

}
//____________________________
myAliFemtoKStarCorrFctn::~myAliFemtoKStarCorrFctn(){
  // destructor
  delete fNumerator;
  delete fDenominator;
  delete fRatio;
...
myAliFemtoKStarCorrFctn& myAliFemtoKStarCorrFctn::operator=(const myAliFemtoKStarCorrFctn& aCorrFctn)
{
  // assignment operator
  if (this == &aCorrFctn)
    return *this;
...
//_________________________
void myAliFemtoKStarCorrFctn::Finish(){
...
//____________________________
AliFemtoString myAliFemtoKStarCorrFctn::Report(){
  // construct report
...
//____________________________
void myAliFemtoKStarCorrFctn::AddRealPair(AliFemtoPair* pair){
  // add true pair
  if (fPairCut)
...
//_______________________________________________________________
  ...
  //  cout << "myAliFemtoKStarCorrFctn::AddRealPair : " << pair->KStar() << " " << tKStar <<
  //" " << pair->track1().FourMomentum() << " " << pair->track2().FourMomentum() << endl;
}

//____________________________
void myAliFemtoKStarCorrFctn::AddMixedPair(AliFemtoPair* pair){
  ...
  if (fPairKinematics) {
    fPairKStar->Fill(pair->KStar(),pair->KOut(),pair->KSide(),pair->KLong());
  }
...
//____________________________
void myAliFemtoKStarCorrFctn::Write(){
...
TList* myAliFemtoKStarCorrFctn::GetOutputList()
{
